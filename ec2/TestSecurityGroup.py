...
def get_or_create_security_group(c, group_name, description=""):
    """
    """
    groups = [g for g in c.describe_security_groups().get('SecurityGroups') if g.get('GroupName') == group_name]
    group = groups[0] if groups else None
    if not group:
        print "Creating group '%s'..."%(group_name,)
        group = c.create_security_group(GroupName=group_name, Description="A group for %s"%(group_name,))
    return group
...
def update_security_group(c, group, expected_rules):
    """
    """
    print 'Updating group "%s"...'%(group.get('GroupName'))
    import pprint
